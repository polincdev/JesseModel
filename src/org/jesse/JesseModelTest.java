...
public class JesseModelTest extends SimpleApplication  implements AnimEventListener {
 ...
 private Vector3f viewDirection = new Vector3f(0,0,1);
  ...
  public void simpleInitApp() {
     ...
     //Basic input
     initKeys();       
     //Main node containing models and lights. Models are added to Scene imported node  
...
@Override
 public void simpleUpdate(float tpf) {
  ...
  void  makeEffects()
  {
    ...
    for(Spatial child: ((Node)modelsNode.getChild("Scene")).getChildren())
  ...
  public void onAction(String binding, boolean value, float tpf){
      ...
      if (value) 
         {
             ...
             left = true;
             if(System.currentTimeMillis()-leftTime<runMarginInMilis)
                doRunStrafeLeft();
             else
               doWalkStrafeLeft();
     ...
     else if (binding.equals("Right"))
       {
         ...
         {
             right = true;
            if(System.currentTimeMillis()-rightTime<runMarginInMilis)
               doRunStrafeRight();
             else
               doWalkStrafeRight();
   ...
   else if (binding.equals("Up"))
     {
      if (value) 
         {
             up = true; 
             if(System.currentTimeMillis()-upTime<runMarginInMilis)
               doRun();
            else
                doWalk();
    ...
    else if (binding.equals("Down") &&  !jump ) 
      {
        ...
        if(jump)
            return;
          jump=true;    
          //doJumpStart provides full jump animation. doJumpMid stats midway and is just faster
          doJumpStart();
          //doJumpMid();
 ...
 void doTurnLeft()
 {
    ...
    if(!left && !right && !up && !down && !jump && !crouch ) 
       setAnimLoop("TurnLeft",1.5f,blendTime);
 } 
 ...
 void doTurnRight()
 {
    ...
    if(!left && !right && !up && !down && !jump && !crouch ) 
       setAnimLoop("TurnRight",1.5f,blendTime);
 } 
 void doStand()
 {
      run=false;
    ...
    void doCrouchToStand()
 {
     ...
     setAnimOnce("CrouchToStand",2.0f,blendTime/2f );
       ...
       if(!simpleControl)
        betterCharacterCtrl.setDucked(false);
 } 
   void doStandToCrouch()
 {
     ...
     setAnimOnce("StandToCrouch",1.0f,blendTime);
       ...
       if(!simpleControl)
        betterCharacterCtrl.setDucked(true);
 } 
  void doCrouch()
 {
      run=false;
 ...
 void doRunStrafeLeft()
 {
      run=true;
       if(!jump)
     setAnimLoop("RunStrafeLeft",1.0f,blendTime);
 } 
  void doRunStrafeRight()
 {
      run=true;
       if(!jump)
     setAnimLoop("RunStrafeRight",1.0f,blendTime);
 } 
 void doRun()
 {
     run=true;
      if(!jump)  
     setAnimLoop("Run",0.9f,blendTime);
 } 
  void doStop()
 {
     run=false;
      if(!jump)  
        setAnimOnce("Stop",1.0f,blendTime*4);
 } 
 ...
 void doWalkStrafeLeft()
 {
       run=false;
        if(!jump)
     setAnimLoop("WalkStrafeLeft",1.0f,blendTime);
 } 
  void doWalkStrafeRight()
 {
       run=false;
        if(!jump)
     setAnimLoop("WalkStrafeRight",1.0f,blendTime);
 } 
 void doWalk()
 {
       run=false;
     if(!jump)
     setAnimLoop("Walk",1.0f,blendTime);
 } 
  void doWalkBack()
 {
       run=false;
      if(!jump)
     setAnimLoop("WalkBack",1.0f,blendTime);
 }  
 void doJumpStart()
 {
      ...
      if(up)
        {
          ...
          if(simpleControl)
            characterCtrl.jump( new Vector3f(0,7,0));
           else
             betterCharacterCtrl.jump(  );
        }
      else      
        setAnimOnce("JumpStart",1.7f,blendTime);
 }
  void doJumpMid()
 {
  ...
  void doJumpEnd()
 {
  ...
  // bulletAppState.setDebugEnabled(true);
   ...
   ((Node)modelsNode.getChild("Scene")).attachChild(jasse);
    ...
    if(simpleControl)
      bulletAppState.getPhysicsSpace().add(characterCtrl);
  else
    {
